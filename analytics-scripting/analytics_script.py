...
def get_most_recent_timestamp(flowId="t035KrLTyPv3jJao4jzwRrSzyV2gU1oK"):
    newest_timestamp_query = {
        "query": {
            "bool": {
                "must": [{"match_phrase": {"flowId": {"query": flowId}}}],
                "filter": [{"match_all": {}}],
            }
        },
        "size": 0,
    ...
    try:
        return res["aggregations"]["types_count"]["hits"]["hits"][0]["sort"][0] - 300000
    except KeyError:
        return int(datetime.now().timestamp()) - 300000
...
def getCommandLineOrDefaultDate(argNum, default):
    try:
        return int(parser.parse(sys.argv[argNum]).timestamp())
...
def send_query_and_evaluate_result(es, query, num_composite_buckets, cmdLineParser):
    try:
        dateStart = int(parser.parse(cmdLineParser.startDate).timestamp())
    except AttributeError:
        dateStart = get_most_recent_timestamp()

    try:
        dateEnd = int(parser.parse(cmdLineParser.endDate).timestamp())
    except AttributeError:
        dateEnd = int(datetime.now().timestamp())

    try:
        flowId = cmdLineParser.flowId
    except AttributeError:
...
cmdLineParser.add_argument("--startDate")
cmdLineParser.add_argument("--endDate")
cmdLineParser.add_argument("--flowId")
args = cmdLineParser.parse_args()

send_query_and_evaluate_result(es, query, 10, parser)
